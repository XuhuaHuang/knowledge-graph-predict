...
class Main(object):
    ...
    def add_model(self):
        model = GPKG_PREDICT(
            self.edge_index, self.edge_type, self.chequer_perm, params=self.p
        )
    ...
    def evaluate(self, split, epoch=0):
        ...
        self.logger.info(
            "[Epoch {} {}]: MRR: Tail : {:.5}, Head : {:.5}, Avg : {:.5}, hits@1 : {:.5}, hits@3 : {:.5}, hits@5 : {:.5}, hits@10 : {:.5}".format(
                epoch,
                split,
                results["left_mrr"],
                results["right_mrr"],
    ...
    def fit(self):
        self.best_val_mrr, self.best_val, self.best_epoch = 0.0, {}, 0.0
        val_mrr = 0
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-name",
        default="testrun_" + str(uuid.uuid4())[:8],
        help="Name of the experiment",
    )

    # Training parameters
    parser.add_argument(
        "-gpu", type=str, default="0", help="GPU to use, set -1 for CPU"
    )
    parser.add_argument(
        "-neg_num",
        dest="neg_num",
    ...
    parser.add_argument(
        "-batch", dest="batch_size", default=128, type=int, help="Batch size"
    )
    ...
    parser.add_argument(
        "-seed", dest="seed", default=41504, type=int, help="Seed to reproduce results"
    )
    parser.add_argument(
        "-restore",
        dest="restore",
    ...
    parser.add_argument(
        "-k_w", dest="k_w", default=10, type=int, help="Width of the reshaped matrix"
    )
    parser.add_argument(
        "-k_h", dest="k_h", default=20, type=int, help="Height of the reshaped matrix"
    )
    parser.add_argument(
        "-num_filt",
        dest="num_filt",
    ...
    parser.add_argument(
        "-ker_sz", dest="ker_sz", default=7, type=int, help="Kernel size to use"
    )
    parser.add_argument(
        "-perm",
        dest="perm",
